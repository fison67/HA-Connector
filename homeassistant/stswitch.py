...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    res_on   = config.get("res_on")
    res_off  = config.get("res_off")
    timeout = config.get(CONF_TIMEOUT)
    json_attrs = config.get(CONF_JSON_ATTRS)

    try:
        switch = STSwitch(name, resource, method, res_on, res_off, timeout, json_attrs, hass)
...
class STSwitch(SwitchDevice):
    ...
    def __init__(self, name, resource, method, res_on, res_off, timeout, json_attrs, hass):
