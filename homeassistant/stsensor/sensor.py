"""
Support for ST Sensors(HA Connector).
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    if username and password:
        ...
        auth = None
    rest = STData(method, resource, auth, headers, payload, verify_ssl)
    rest.update()

    add_devices([STSensor(
        ...
        if self._json_attrs:
            ...
            try:
                ...
                if isinstance(json_dict, dict):
                    attrs = {k: json_dict['attributes'][k] for k in self._json_attrs
                             if k in json_dict['attributes']}
